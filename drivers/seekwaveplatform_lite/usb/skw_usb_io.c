...
static void usb_setup_service_devices(void)
{
	...
	if(usb_ports[1]->pdev){
		if(wifi_data_pdev==NULL) {
			/*
			 * The swt6621s_wifi core matches the sv6621s_wirelessN
			 * pdev by name and shares no symbols with this module,
			 * so depmod cannot order them. Make sure the core is
